...
function OfferTAC({ id }) {
  ...
  const handleSetOfferTaC = (payload) => {
    // Update the offerTermsAndConditions state by adding the new payload
    setOfferTermsAndConditions([...offerTermsAndConditions, { ...payload }]);
  };

  const createOfferData = useSelector((state) => ({
    activeSection: state.createOffer.createOfferActiveSection,
    offerTaC: state.createOffer.offerTermsAndConditions,
  }));
  const dispatch = useDispatch();

  const { activeSection, offerTaC } = createOfferData;
  ...
  const handleNewTac = () => {
    if (newTacTitle && newTacDetail) {
      dispatch(
        setOfferTaC({
          id: offerTaC?.length + 1,
          title: newTacTitle,
          content: newTacDetail,
        })
      );
    }
    setNewTacTitle("");
        ...
        <div className="w-full space-y-4">
          ...
          <div className="flex flex-col space-y-1 w-full">
            <div className="rounded-sm bg-[#1C2225] px-4 py-2 h-[35px] w-full flex items-center space-x-4 justify-start">
              ...
              <input
                value={newTacTitle}
                onInput={(e) => setNewTacTitle(e.currentTarget.value)}
                type="text"
            ...
            <div className="rounded-sm bg-[#1C2225] p-4 h-full w-full flex items-start space-x-4 min-h-[74px] justify-start">
              ...
              <textarea
                value={newTacDetail}
                onInput={(e) => setNewTacDetail(e.currentTarget.value)}
                rows={4}
                type="text"
                className="w-full h-full border-none outline-none bg-transparent font-karla font-bold text-sm text-[#A6A6A6] placeholder:text-[#a6a6a6]"
