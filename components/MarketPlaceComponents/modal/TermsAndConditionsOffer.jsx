...
export default function TermsAndConditionsOffer({
  data,
  isSuccess,
  isFetching,
  isLoading,
  ...
  const [
    sellerInitiateRequest,
    { data: dataSeller, isError, isLoading: IsloadingSeller, error },
  ] = useSellerInitiateRequestMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await sellerInitiateRequest(sellerParams).unwrap();
            ...
            <Box className="py-4 md:py-6 bg-secondary shadow-sm xl:shadow-md rounded-sm space-y-4 text-secondary">
              ...
              >
                ...
                {isLoading || isFetching
                  ? "Loading........"
                  : isSuccess &&
                    data?.response?.data?.sellerterm?.map((terms, index) => (
                      <div
                        key={index}
                ...
                >
                  {IsloadingSeller ? "loading....." : " Agree and Continue"}
                </button>
              </form>
            </Box>
          </div>
