...
function MenuField({ options, value, setValue, label, width = 360, placeholder = 'Select Category', noKey = false }) {
    const [anchorEl, setAnchorEl] = useState(null);
    const [optionLabel, setOptionLabel] = useState(null)

    useEffect(() => {
        const filterOption = options.filter(opt => opt['value'] === value)
        setOptionLabel(filterOption['label'])
    }, [value])
    ...
    return (
        ...
        >
            <Box
                id={`${label} -button`}
                aria-controls={open ? `${label} -button` : undefined}
                aria-haspopup="true"
                aria-expanded={open ? 'true' : undefined}
                onClick={openOptions}
                className="w-full px-4 py-3.5 rounded border border-border flex items-center justify-start cursor-pointer"
            >
                {value ? (
                    <p className="capitalize font-karla font-bold text-xs md:text-sm xl:text-[16px] text-right flex-wrap">
                        {optionLabel}
                    </p>
                ) : (
                    <div className="w-full flex gap-2 items-center justify-between px-2">
                        <p className="w-full  font-karla font-bold text-sm md:text-lg ">
                            {placeholder}
                        </p>
                        <p className=''>
                            <span>
                    ...
                    >
                        {options.map((option) => (
                            <MenuItem
                                sx={{ width: width }}
                                onClick={() => {
                                    setValue(!noKey ? option.value : option);
                                    setAnchorEl(null);
                                }}
                                key={!noKey ? option.value : option}
                            >
                                <div>
                                    <p className="font-rubik capitalize text-sm font-medium text-secondary ">
                                        {!noKey ? option.label : option}
                                    </p>
                                </div>
                            </MenuItem>
                        ))}
