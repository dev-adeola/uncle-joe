"use client";

import { Box } from "@mui/material";
import React, { useState, useEffect, useRef } from "react";
import { useParams } from "next/navigation";
import useGetUpdate from "./useGetUpdate";
const status = ["makePayment", "confirmAndPay", "completed", "cancelled"];

const transactionStatuses = [
  {
    info: "Kindly discus all necessary details while <freelance> fulfill the payment.",
    title: "Awaiting payment",
    class: "#816F38",
  },
  {
    info: "Kindly verify the payment at your end before releasing. Beware of fake payment notifications!",
    title: "Confirm and release",
    class: "#505050",
  },
  {
    info: "Your payment has been released and added to your account balance",
    title: "Completed",
    class: "#00B172",
  },
  {
    info: "This transaction has been canceled and Ratefy escrow has refunded the amount held.",
    title: "Canceled",
    class: "#505050",
  },
];

function BuyerChatHead(headStatus) {
  const params = useParams();
  const { getUpdate } = useGetUpdate(params.acceptance_id + '-' + params.session_id);
  const index = useRef(0);

  if(getUpdate === undefined || getUpdate === null) {
    if(headStatus.status.proof_of_payment === "void") {
      index.current = 0;
    }else if(headStatus.status.proof_of_payment !== "void" && headStatus.status.proof_of_payment_status === "void") {
      index.current = 1;
    }else if(headStatus.status.proof_of_payment_status === "accept") {
      index.current = 2;
    }
  }else {
    if(parseInt(getUpdate.updateState) > 0) {
      index.current = parseInt(getUpdate.updateState);
    }else {
      index.current = 0;
    }
  }

  return (
    <>
      <Box className="w-full" bgcolor={transactionStatuses[index.current].class}>
        <Box className="w-full md:space-y-2 h-fit max-h-[130px] text-white p-4 md:px-8 ">
          <div className="hidden md:block">
            <p className="uppercase text-sm font-medium font-rubik">
              transaction status
            </p>
          </div>
          <div className="flex w-full h-full items-center ">
            {/*  */}
            <div className="space-y-2 w-full flex-1">
              <p className="capitalize text-[20px] font-bold font-rubik">
                {transactionStatuses[index.current].title}
              </p>
              <p className="max-w-[220px] text-xs font-karla font-medium md:text-sm">
                {transactionStatuses[index.current].info}
              </p>
            </div>
            {/*  */}
            <div className="items-center flex space-x-2 text-black">
              <div className="w-[23px] h-[23px] rounded-[3px] bg-white text black justify-center items-center flex">
                <p>1</p>
              </div>
              <div className="w-[23px] h-[23px] rounded-[3px] bg-white text black justify-center items-center flex">
                <p>2</p>
              </div>
              <div className="h-[23px] rounded-[3px]">
                <p className="text-semibold">:</p>
              </div>
              <div className="w-[23px] h-[23px] rounded-[3px] bg-white text black justify-center items-center flex">
                <p>4</p>
              </div>
              <div className="w-[23px] h-[23px] rounded-[3px] bg-white text black justify-center items-center flex">
                <p>5</p>
              </div>
            </div>
          </div>
        </Box>
      </Box>
    </>
  );
}

export default BuyerChatHead;
