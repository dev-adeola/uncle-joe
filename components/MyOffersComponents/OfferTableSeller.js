...
export default function MyOfferSellerTableOffer({
  dataSellerOffer,
  ...
  const handleClick = (event, row) => {
    ...
    setSellerIds({ uuid: row?.uuid, id: row?.id });
  };
  ...
  const [pausesSellerOffer, { data, isError, isLoading, error }] =
    usePausesSellerOfferMutation();
  const [
    activateSellerOffer,
    {
  ...
  const handlePauseBuyerOffer = async (e) => {
    try {
      const response = await pausesSellerOffer(sellerIds);
  ...
  const handleActivateBuyerOffer = async (e) => {
    try {
      const response = await activateSellerOffer(sellerIds);
      console.log({ response });
      if (response?.data?.status == 200) {
                    ...
                    <TableRow key={row?.id}>
                      ...
                      <TableCell align="center">
                        <div className="text-white px-2">
                          {row.sellerofferrequirement?.map(
                            (requirement, index) => (
                              <span key={index}>
                                {requirement.requirement.requirement}
                                {index !==
                                  row.sellerofferrequirement.length - 1 && ", "}
                              </span>
                            )
                          )}
                        </div>
