...
import TermsAndConditionsBuyerOffer from "../MarketPlaceComponents/modal/TermsAndConditionsBuyerOffer";
import { calculateAmountToRecieve } from "@/utils";

function CalculatePageBuyer({ buyerId }) {
  const [open, setOpen] = useState(false);
  const handleOpen = () => setOpen(true);
  const handleClose = () => setOpen(false);
  const [formValues, setFormValues] = useState({});
  const { data, isError, error, isSuccess, isFetching, isLoading } =
    useFetchTermsAndConditionsBuyerOffersQuery(buyerId);
  const {
    data: rateData,
...
export default CalculatePageBuyer;
